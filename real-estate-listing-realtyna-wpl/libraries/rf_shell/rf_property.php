...
class wpl_rf_property
{
	...
	private function map_listing($listing)
	{
		...
		if ($listing_id) {
			wp_cache_set($cacheKey, $listing_id, 'wpl_rf_property', static::CACHE_TTL);
			return $listing_id;
		}
	...
	private function map_property_type($property_type)
	{
		...
		if ($property_type_id) {
			wp_cache_set($cacheKey, $property_type_id, 'wpl_rf_property', static::CACHE_TTL);
	...
	public function map_select_type($fieldId, $metaValue)
	{
		...
		foreach ($options['params'] as $item) {
			if ($metaValue == $item['value']) {
				wp_cache_set($cacheKey, $options, 'wpl_rf_property', static::CACHE_TTL);
				return $item['key'];
			}
	...
	public function load_meta_mapping($mapping)
	{
		...
		if (!$found) {
			...
			wp_cache_set($flexCacheKey, $existColumns, 'wpl_rf_property', static::CACHE_TTL);
		} else {
			$existColumns = $found;
		}
		...
		if (!$found) {
			...
			wp_cache_set($cacheKey, $db_categories, 'wpl_rf_property', static::CACHE_TTL);
		} else {
			$db_categories = $found;
		}
	...
	public function rf_after_mapping($posts)
	{
		...
		if (!$found) {
			...
			wp_cache_set($cacheKey, $fields, 'wpl_rf_property', static::CACHE_TTL);
		} else {
			$fields = $found;
		}
		...
		foreach ($posts as &$post) {
			...
			foreach ($post->meta_data as $metaKey => $metaValue) {
				...
				if (in_array($newKey, array_keys($specialFields))) {
					...
					if ($field['type'] == 'listings') {
						...
						$field['type'] == 'select' && (
						(is_array($post->meta_data[$metaKey]) and !empty($post->meta_data[$metaKey])) or
						strlen($post->meta_data[$metaKey] ?? '') > 0
						)
					) {
						$post->meta_data[$newKey] = $this->map_select_type($field['id'], is_array($post->meta_data[$metaKey]) ? array_shift($post->meta_data[$metaKey]) :$post->meta_data[$metaKey]);
						continue;
			...
			if (empty($post->meta_data['kind'])) {
				$post->meta_data['kind'] = 0;
			}
		}
		unset($post);

		$saved_properties = $this->preload_saved_properties($posts);
		...
		foreach ($posts as &$post) {
			...
			$lookup_key = static::saved_property_key($kind, $mls_id, $source);
			$saved_property = $saved_properties[$lookup_key] ?? null;
			if (empty($saved_property)) {
				...
				$saved_properties[$lookup_key] = $property;
			} else {
	...
	/**
	 * Key used to index preloaded `#__wpl_properties` rows.
	 *
	 * Normalised to match how MySQL compared these columns in the per-listing
	 * query this replaced: `kind` is numeric, and the default collation makes
	 * `mls_id` / `source` case- and trailing-space-insensitive. Without that, a
	 * row stored as 'ABC123' would not be found under a feed value of 'abc123'
	 * and the listing would be recreated as a duplicate.
	 */
	private static function saved_property_key($kind, $mls_id, $source)
	{
		return intval($kind)
			. '|' . strtolower(rtrim((string)$mls_id))
			. '|' . strtolower(rtrim((string)$source));
	}

	private function preload_saved_properties($posts)
	{
		$groups = [];
		foreach ($posts as $post) {
			if (empty($post->meta_data)) {
			...
			$mls_id = $post->meta_data['mls_id'] ?? null;
			$kind = $post->meta_data['kind'] ?? 0;
			$source = $post->meta_data['source'] ?? '';

			$group_key = intval($kind) . '|' . strtolower(rtrim((string)$source));
			if (!isset($groups[$group_key])) {
				$groups[$group_key] = ['kind' => $kind, 'source' => $source, 'mls_ids' => []];
			}
			// Keyed to de-duplicate repeated mls_ids within the batch.
			$groups[$group_key]['mls_ids'][(string)$mls_id] = $mls_id;
		}

		$saved = [];
		foreach ($groups as $group) {
			$kind = intval($group['kind']);
			$source = wpl_db::escape($group['source']);

			/** chunked so the statement stays a sane size on marker requests **/
			foreach (array_chunk($group['mls_ids'], 500) as $chunk) {
				$escaped = [];
				foreach ($chunk as $mls_id) {
					$escaped[] = "'" . wpl_db::escape($mls_id) . "'";
				}

				$query = "SELECT * FROM `#__wpl_properties` WHERE kind = '" . $kind
					. "' AND `source` = '" . $source
					. "' AND mls_id IN (" . implode(',', $escaped) . ")"
					. " ORDER BY id ASC";
				$rows = wpl_db::select($query, 'loadAssocList');
				if (empty($rows)) {
					continue;
				}

				foreach ($rows as $row) {
					$key = static::saved_property_key($kind, $row['mls_id'], $group['source']);
					// The per-listing query used loadAssoc, i.e. the first match.
					// Keep that when duplicates exist.
					if (!isset($saved[$key])) {
						$saved[$key] = $row;
					}
				}
	...
	public function getOpenDates($refId)
	{
		...
		try {
			$items = $this->doRfRequest('OpenHouse', [
				...
				['method' => 'where', 'field' => 'OpenHouseDate', 'operator' => 'ge', 'value' => gmdate('Y-m-d', strtotime('-1 day'))],
			]);
			...
			wp_cache_set($cacheKey, ['result' => $openDates, 'timestamp' => time()], 'wpl_rf_property', 7200);
			return $openDates;
		} catch (Exception $e) {
			return [];
		}
	...
	private function update_existing_property($property_id, $saved_property, $ref_id, $rfData)
	{
		if (!empty($ref_id) && (empty($saved_property['ref_id']) || $ref_id != $saved_property['ref_id'])) {
			...
			wp_cache_delete("get_cloud_post_id-$property_id", 'wpl_rf_property');
		}
		...
		if (empty($saved_property['last_sync_date']) ||
			...
			$update['last_sync_date'] = gmdate('Y-m-d H:i:s');
		}
		if (!empty($update)) {
	...
	public function get_cloud_post_id($property_id)
	{
		...
		wp_cache_set($cacheKey, $result, 'wpl_rf_property', static::CACHE_TTL);
		if (wpl_global::is_multisite()) {
			$fs = wpl_sql_parser::getInstance();
			$fs->enable();
		}
	...
	{
		$last_month = gmdate('Y-m-d H:i:s', time() - 2592000);
		$last_week = gmdate('Y-m-d H:i:s', time() - 604800);
		if (wpl_global::is_multisite()) {
			$fs = wpl_sql_parser::getInstance();
			$fs->disable();
		}
